...
const CreateBook = () => {
  ...
  const handleSaveBook = async (e) => {
    ...
    // Publish Year validation: check if it's a 4-digit number
    if (!/^\d{4}$/.test(publishyear)) {
      enqueueSnackbar('Invalid publish year. Please enter a 4-digit year.', { variant: 'error' });
      return;
    }

    // Email validation: check if it's a valid email
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(yemail)) {
      enqueueSnackbar('Invalid email address. Please enter a valid email.', { variant: 'error' });
      return;
    }
    ...
    setLoading(true);
    try {
      await axios.post('http://localhost:5000/books/addnotes', JSON.stringify(data), {
        headers: {
          "Authorization": ` ${token}`,
          "Content-Type": "application/json"
        },
